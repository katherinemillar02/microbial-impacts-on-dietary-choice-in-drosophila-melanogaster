...
combined_m <- fourone_onefour_male %>% 
  ...
  ## Data Analysis 
       ...
       #### DATA ANALYSIS WITH "DIET" AS A WHOLE VARIABLE
...
#### testing different models, with the new model design 

# Model 1 
#### Poisson GLMM ####
glmm.m.4choice.2 <- glmmTMB(fly_numbers ~ 
                              ratio * condition * block 
                         + (1 | block / plate) + (1 | block / observation), family = poisson, data = combined_m_split)
...
#### Testing for the best model with diet split 

#### Poisson GLMM ####
glmm.vf.4choice.2 <- glmmTMB(fly_numbers ~ 
                              ratio * condition * block 
                            + (1 | block / plate) + (1 | block / observation), family = poisson, data = combined_vf_split)

## Assumption checks for glmm.m.4choice.2
testDispersion(glmm.vf.4choice.2)
## Looks VERY overdispersed? 


simulationOutput <- simulateResiduals(fittedModel = glmm.vf.4choice.2, plot = T)
  #### Model looks sort of okay? 

performance::check_model(glmm.vf.4choice.2, check = c("qq"))
## Does not show qq results 

performance::check_model(glmm.vf.4choice.2)
## Not sure what I am looking for, but I think the assumptions look ok? 

check_overdispersion(glmm.vf.4choice.2)
# Overdispersion detected very muchhhh

check_zeroinflation(glmm.vf.4choice.2)
# No zero inflation¡¡¡¡!!!!



#### Poisson Zero Inflated ####
zi.pois.vf.4choice.2 <- zeroinfl(fly_numbers ~ 
                                   ratio * condition * block 
                                  | ratio * condition * block
                                 , dist = "poisson", link = "logit", data = combined_vf_split)


#### Negative Binomial Zero Inflated ####
zi.nb.m.4choice.2 <- zeroinfl(fly_numbers ~ 
                                ratio * condition * block
                              | ratio * condition * block
                              , dist = "negbin", link = "logit", data = combined_m_split)
...
#### Zero-Inflated Negative Binomial has lowest AIC





# Model Check 1 - 3-way interaction effect
zi.nb..4choice.2 <- zeroinfl(fly_numbers ~ 
                                ratio * condition * block
