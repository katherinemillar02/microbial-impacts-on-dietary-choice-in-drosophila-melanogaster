...
#### Upload data
four_to_one_OvoD1_2 <- read_excel("data/OvoD1-4_1-median_2.xlsx")

## Making the data long 
four_to_one_OvoD1_2_long <- four_to_one_OvoD1_2   %>% 
  pivot_longer(cols = ("4:1 Conditioned":"4:1 Unconditioned"), names_to = "diet", values_to = "fly_numbers")

## Visualising the data 
four_to_one_OvoD1_2_plot <- four_to_one_OvoD1_2_long   %>% 
  ggplot(aes(x = diet, y = fly_numbers, fill = diet))+ 
  geom_boxplot()+
  theme_classic()+
  scale_fill_brewer(palette = "Set2")+
...
## Statistical analysis ----
# First testing a linear model 
four_to_one_OvoD1_2_lm <- lm(fly_numbers ~  diet, data = four_to_one_OvoD1_2_long)

# Assumption Checking of the model 
performance::check_model(four_to_one_OvoD1_2_lm, check = c("qq")) # qq looks okay, dots are a bit spaced out, could be a lot better. 
performance::check_model(four_to_one_OvoD1_2_lm, check = c("homogeneity")) # quite windy
performance::check_model(four_to_one_OvoD1_2_lm, check = c("linearity")) # quite windy, not the best
performance::check_model(four_to_one_OvoD1_2_lm, check = c("outliers")) # don't see anything particularly awful. 




# Trying a generalised linear model
four_to_one_OvoD1_2_glm_1 <- glm(fly_numbers ~  diet, family = poisson(link = "log"), four_to_one_OvoD1_2_long)
...
# summary function, shows t test
summary(four_to_one_OvoD1_2_glm_2)

# using anova 
anova(four_to_one_OvoD1_2_glm_2)

# emmeans for tukey analysis 
emmeans::emmeans(four_to_one_OvoD1_2_glm_2, pairwise ~ diet)
## This shows there is not a significant difference between 4:1 Conditioned and 4:1 Unconditioned. 
