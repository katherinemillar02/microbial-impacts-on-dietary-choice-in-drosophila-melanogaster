...
# setting file paths for the two datasets, one from the controlled experiment, and one from the uncontrolled experiment. 
...
timecourse_data <- function(path, sex_cols = c("females", "males"), cols = "vial") {
  read_excel(path) %>% 
    pivot_longer(cols = all_of(sex_cols),  # making the sex columns into one column
                 names_to = "sex",
                 values_to = "count") %>% 
    drop_na(count) %>% 
    uncount(count)
...
# combining paths with the written function 
all_timecourse_data <- purrr::imap_dfr(paths_adult, ~ 
                                   timecourse_data(.x) %>% 
                                   mutate(density_condition = .y)) # mutating a function called density_conditioning, based on what the paths were called above. 
...
##### ##### CUREENT DATA READ IN ===
# This is the current data read in before it has been turned into a function. 
# I am keeping it here for now in case anything I have been working on fails.
