...
simulation_Output <- simulateResiduals(fittedModel = glmm.bin.m.ovi.2choice, plot = F)
plot(simulation_Output) 
...
glmm.bin.m.ovi.2choice <- glmer(cbind(Conditioned, Unconditioned) ~ ratio + block + (1|plate/block)  , family = binomial, data = df2_male_oviposition)
...
emmeans::emmeans(glmm.bin.m.ovi.2choice, ~ ratio, type = "response")
